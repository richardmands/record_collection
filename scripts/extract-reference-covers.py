...
manifest=json.loads((root/'data/research/cover-crops.json').read_text(encoding='utf-8-sig'))
size=1200
...
for c in manifest['covers']:
 ...
 out.save(root/f"public/covers/{c['id']}_reference_crop.jpg",quality=92,optimize=True)
 print(c['id'],c['photo'])
# Contact sheet for visual QA only.
sheet=Image.new('RGB',(990,1050),'#eee');draw=ImageDraw.Draw(sheet)
