...
export function buildCollection(albumRows, trackRows, root) {
  ...
  for (const a of albums) {
    ...
    for (const key of ['discogsUrl','coverSource']) if (a[key]) {
    ...
    if (a.coverImage) {
      ...
      if (!a.coverSource || !['discogs','retailer'].includes(a.coverStatus)) throw new Error(`Record ${a.id}: only sourced online artwork may be published`);
